...
namespace Suggession.Services
{
    public interface IGHRService
    {
        ...
        Task<object> GetReportInfoH1(int accountId);
        Task<object> GetData();
        Task<object> GetReportInfoH2(int accountId);
        Task<Byte[]> ExportExcel(int accountId);
        Task<bool> ReportUpdateComment(List<CommentUpdateDto> model);
        Task<bool> ReportUpdateAtScore(List<AtScoreUpdateDto> model);
        Task<bool> ReportUpdateSpeComment(List<CommentUpdateDto> model);
        Task<object> GetAllKPIScoreL0ByPeriod(int period);
    }
    public class GHRService : IGHRService
    {
        ...
        public async Task<bool> ReportUpdateComment(List<CommentUpdateDto> model)
        {
            ...
            try
            {
                ...
                {
                    var transfer = _repoComment.FindById(item.ID);
                    transfer.Content = item.Content;

                    _repoComment.Update(transfer);
        ...
        public async Task<bool> ReportUpdateAtScore(List<AtScoreUpdateDto> model)
        {
            ...
            try
            {
                foreach (var item in model)
                {
                    var transfer = _repoAttitudeScore.FindById(item.ID);
                    transfer.Point = item.Point;

                    _repoAttitudeScore.Update(transfer);
                    await _unitOfWork.SaveChangeAsync();
                }
                return true;
            }
        ...
        public async Task<bool> ReportUpdateSpeComment(List<CommentUpdateDto> model)
        {
            ...
            try
            {
                foreach (var item in model)
                {
                    var transfer = _repoContribution.FindById(item.ID);
                    transfer.Content = item.Content;

                    _repoContribution.Update(transfer);
                    await _unitOfWork.SaveChangeAsync();
                }
                return true;
            }
        ...
        public async Task<object> getObjectKPIH1(int accountId)
        {
            ...
            var halfYearModel = (await _repoPeriod.FindAll(x => x.PeriodType.Code == SystemPeriod.HalfYear && x.Value == HalfYear.H1).ToListAsync())
             .Select(x => new
             {
                 x.Title,
                 x.PeriodTypeId,
            ...
            var monthsOfCurrentHalfYear = halfYearModel.Months;
        ...
        public async Task<object> getObjectKPIH2(int accountId)
        {
            var date = DateTime.Today;
            var month = date.Month;
            int currentHalfYear = month <= 6 && month >= 1 ? 1 : 2;
            ...
            // Lấy settings của quý và nửa năm
            var halfYearModel = (await _repoPeriod.FindAll(x => x.PeriodType.Code == SystemPeriod.HalfYear && x.Value == HalfYear.H2).ToListAsync())
             .Select(x => new
             {
                 x.Title,
                 x.PeriodTypeId,
        ...
        public async Task<object> GetReportInfoH2(int accountId)
        {
            throw new NotImplementedException();
        }
        ...
        public Task<object> GetReportInfoH1(int accountId)
        {
            throw new NotImplementedException();
        }
    }
