...
@Component({
  selector: 'app-satisfiedSpokermanModal',
  templateUrl: './satisfiedSpokermanModal.component.html',
  styleUrls: ['./satisfiedSpokermanModal.component.scss']
})
export class SatisfiedSpokermanModalComponent implements OnInit {
  @Input() data: any;
  @Input() nameTitle: any;
  ...
  ngOnInit() {
    this.titleModal = this.data.statusName
  ...
  savePlan() {
    ...
    if (this.planIdeaDataAddOrUpdate.statusPlanID === 0 && this.isActionBegin === false && this.planIdeaData.length === 0) {
      this.alertify.warning('Please add Plan');
      return;
    } 

    if (this.planIdeaDataAddOrUpdate.statusPlanID === 0 && this.isActionBegin === true) {
  ...
  getIdeaHisById(id) {
    this.todolist2Service.getIdeaHisByIdWithoutFactoryHead(id, this.locale).subscribe((res: any) => {
      this.dataHis = res
    })
  }

  getPlanIdea(id) {
    this.todolist2Service.getPlanIdea(id).subscribe((res) => {
